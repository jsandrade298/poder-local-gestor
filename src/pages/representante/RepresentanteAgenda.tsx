...
const RepresentanteAgenda = () => {
  ...
  const { data: usuarios } = useQuery({
    queryKey: ["usuarios-agenda-rep", profile?.tenant_id],
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      await supabase.from("notificacoes").insert({
        ...
        url_destino: `/rep/agenda?agenda=${agenda.id}`,
        lida: false
      });
      ...
      if (agenda && user?.id) {
        ...
        if (agenda.solicitante_id !== user.id) {
          await supabase.from("notificacoes").insert({
            ...
            url_destino: `/rep/agenda?agenda=${agendaId}`,
          });
        }
...
export default RepresentanteAgenda;
